(() => {
  ...
  const HISTORY_INTRO_KEY='cantinho_petisco_history_intro_seen_v1';
  ...
  function syncPageInteractionLock(){
    const modalOpen=[els.itemDialog,els.checkout,els.privacy,els.historyIntroDialog,els.historyDialog,els.repeatChoiceDialog].some(d=>d?.open);
  ...
  function historyIntroSeen(){try{return localStorage.getItem(HISTORY_INTRO_KEY)==='1';}catch{return true;}}
  function syncHistoryAttention(){const b=$('#openHistoryTop');if(!b)return;b.classList.toggle('history-attention',!historyIntroSeen());}
  function markHistoryIntroSeen(){try{localStorage.setItem(HISTORY_INTRO_KEY,'1');}catch{}syncHistoryAttention();}
  function openHistoryDirect(){closeCart();renderHistory();openAppDialog(els.historyDialog);}
  function openHistory(){closeCart();if(!historyIntroSeen()&&els.historyIntroDialog){openAppDialog(els.historyIntroDialog);return;}openHistoryDirect();}
  ...
  function initEvents(){
    [els.itemDialog,els.checkout,els.privacy,els.historyIntroDialog,els.historyDialog,els.repeatChoiceDialog].forEach(d=>d?.addEventListener('close',syncPageInteractionLock));
    ...
    $('#openCartTop').onclick=openCart;els.mobileCartBar.onclick=openCart;$('#openHistoryTop')?.addEventListener('click',openHistory);$('#openHistoryFooter')?.addEventListener('click',openHistory);$('#historyIntroContinue')?.addEventListener('click',()=>{markHistoryIntroSeen();els.historyIntroDialog?.close();setTimeout(openHistoryDirect,0);});$('#closeHistoryDialog')?.addEventListener('click',()=>els.historyDialog?.close());$('#closeHistoryBottom')?.addEventListener('click',()=>els.historyDialog?.close());
  ...
  initEvents();syncHistoryAttention();loadMenu();
})();
