(() => {
  ...
  function openGroup(id){const g=state.groupMap.get(id);if(!g)return;state.specialOverride=null;state.selectedVariants=sortVariants(g.variants);state.selected=chooseDefaultVariant(state.selectedVariants);state.selectedQty=1;resetSelectedOptions();els.itemNote.value='';renderItemDialog();els.itemDialog.showModal();}
